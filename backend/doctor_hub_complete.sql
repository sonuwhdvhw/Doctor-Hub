-- =============================================================================
-- Doctor Hub — Complete Supabase Database Schema
-- =============================================================================
-- Run this ENTIRE file once in Supabase SQL Editor:
--   https://supabase.com/dashboard → SQL → New query → Paste → Run
--
-- Covers ALL app features:
--   • Auth (Patient, Doctor, Assistant, Admin, Super Admin)
--   • Doctor profiles, clinics, schedules
--   • Appointments & payment verification
--   • Medical history & prescriptions
--   • Patient reports upload & doctor-patient messaging
--   • Legacy CareLink tables (backward compatibility)
-- =============================================================================
...
CREATE TABLE IF NOT EXISTS users (
    ...
    role TEXT NOT NULL DEFAULT 'patient'
        CHECK (role IN ('patient', 'doctor', 'assistant', 'admin', 'superadmin')),
    image TEXT,
...
-- ─── Payments (screenshot verification by assistant) ───────────────────────────
...
-- ─── Prescriptions (immutable after creation) ────────────────────────────────
...
-- ─── Legacy CareLink Tables (optional — kept for backward compatibility) ─────
...
CREATE INDEX IF NOT EXISTS idx_doctor_appointments_patient ON doctor_appointments(patient_id);
...
CREATE INDEX IF NOT EXISTS idx_medical_history_doctor ON medical_history(doctor_id);
CREATE INDEX IF NOT EXISTS idx_medical_history_appointment ON medical_history(appointment_id);
...
-- ─── Done ────────────────────────────────────────────────────────────────────
-- Next steps:
--   1. Copy SUPABASE_URL and SUPABASE_SERVICE_KEY to backend/.env
--   2. Run: cd backend && npm run seed  (creates test accounts)
